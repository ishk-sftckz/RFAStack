...
This extends the schema file from the [folder structure example](../docs/folder-structure.md#use-zod-schemas-and-infer-their-types); keep its existing `orderStatusSchema` definition when adding `orderSummarySchema`. Zod checks values at runtime and infers the corresponding TypeScript type. [Zod basics](https://zod.dev/basics)
...
`requireAccount` is the identity feature’s public server operation for verifying the session and resolving an account the caller may use. The query calls it internally, so each caller receives the same protection. A validated ID alone does not authorize a read. The [protected-resources guide](../docs/protected-resources.md) explains where those checks belong.
...
Next: [protect reads and mutations at the feature boundary](../docs/protected-resources.md).
