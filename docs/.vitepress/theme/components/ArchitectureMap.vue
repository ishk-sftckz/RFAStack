...
<template>
  <figure
    ...
    tabindex="0"
    :aria-label="t('Scrollable RFAStack architecture map', 'Peta arsitektur RFAStack yang dapat digulir')"
  >
    <svg class="architecture-map" viewBox="0 0 960 590" role="img" :aria-label="t('RFAStack architecture map', 'Peta arsitektur RFAStack')" xmlns="http://www.w3.org/2000/svg">
      ...
      <desc>{{ t('Requests enter through src/app and delegate to a feature in src/features. Feature code may call integrations in src/platform and generic primitives in src/shared.', 'Request masuk melalui src/app lalu diteruskan ke fitur di src/features. Kode fitur boleh memanggil integrasi di src/platform dan komponen dasar generik di src/shared.') }}</desc>
      <defs>
      ...
      <text x="34" y="42" class="map-plate">{{ t('CALL FLOW · OWNERSHIP STAYS WITH BUSINESS CAPABILITIES', 'ALUR PANGGILAN · FITUR MEMILIKI PERILAKU BISNIS') }}</text>
      ...
      <g class="map-node map-node--feature">
        ...
        <text x="160" y="250" class="map-node__mark">{{ t('F / BUSINESS CAPABILITIES', 'F / KAPABILITAS BISNIS') }}</text>
        ...
        <text x="800" y="382" text-anchor="end" class="map-node__note">{{ t('OWNS THE BEHAVIOR', 'MEMILIKI PERILAKU') }}</text>
      </g>
      ...
      <g class="map-node map-node--shared">
        ...
        <text x="546" y="501" class="map-node__mark">{{ t('S / GENERIC PRIMITIVES', 'S / KOMPONEN DASAR GENERIK') }}</text>
    ...
    <figcaption><strong>{{ t('Text alternative:', 'Alternatif teks:') }}</strong>{{ t(' requests enter through ', ' request masuk melalui ') }}<code>src/app</code>{{ t(' and delegate to feature-owned behavior in ', ' lalu diteruskan ke perilaku milik fitur di ') }}<code>src/features</code>{{ t('. Features may call integrations in ', '. Fitur boleh memanggil integrasi di ') }}<code>src/platform</code>{{ t(' and generic primitives in ', ' dan komponen dasar generik di ') }}<code>src/shared</code>.</figcaption>
  </figure>
</template>
