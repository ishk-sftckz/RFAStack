...
test('shared cache reuse and private browser reuse have different lifetimes', async ({ page }) => {
  ...
  await expect.poll(() => count('suggested-reorders', 'company-b')).toBeGreaterThan(0)
  const shared = count('orders', 'company-b')
  const personal = count('suggested-reorders', 'company-b')
  await page.getByRole('link', { name: 'B2B ordering', exact: true }).click()
  await expect(page).toHaveURL((url) => url.pathname === '/')
  await page.goBack()
  await expect(page.getByText('Signed in as bob')).toBeVisible()
  expect(count('suggested-reorders', 'company-b')).toBe(personal)
  await page.reload()
  await expect(page.getByText('Signed in as bob')).toBeVisible()
  await expect.poll(() => count('suggested-reorders', 'company-b')).toBeGreaterThan(personal)
  expect(count('orders', 'company-b')).toBe(shared)
})
