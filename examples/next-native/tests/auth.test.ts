...
test('membership verification and its wrapper reject denied access and check each invocation', async () => {
  const operation = vi.fn(async (member: Membership, orderId: string, includeItems: boolean) => ({
  ...
  await expect(read(new Headers(), 'order-1', true)).rejects.toMatchObject({ status: 401 })
  expect(operation).not.toHaveBeenCalled()
  ...
  try {
    ...
    await expect(read(headers, 'order-1', true)).rejects.toMatchObject({ status: 403 })
    expect(operation).not.toHaveBeenCalled()

    await database
      .insert(membership)
      .values({ userId: id, scopeId: 'test-scope', role: 'customer' })
    await expect(requireMembership(headers)).resolves.toMatchObject({
      userId: id,
      scopeId: 'test-scope',
    })
    await expect(read(headers, 'order-1', true)).resolves.toEqual({
      scopeId: 'test-scope',
      orderId: 'order-1',
      includeItems: true,
    })
    ...
    await expect(read(headers, 'order-2', false)).resolves.toEqual({
    ...
    const failingRead = withMembership(async () => {
      throw failure
    })
    await expect(failingRead(headers)).rejects.toBe(failure)
    ...
    await expect(read(headers, 'order-3', true)).rejects.toMatchObject({ status: 401 })
    expect(operation).toHaveBeenCalledTimes(2)
  } finally {
